...
Untrusted input is a policy file; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a policy file
- **Closed type:** `type Decision = ApplyRule(PolicyRule) | IgnoreRule` where `PolicyRule` is closed
- **Sink / capability:** `grant applyPolicy` → `privileged { applyPolicy(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/config-policy/demo.sh
```
...
- `config-policy_safe.eide` — the correct design.
- `config-policy_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
